...
<figure>
  <img src="/static/assets/ae-vs-vae.png" width=700px>
  <figcaption>Comparison of a latent space code (attribute) in an autoencoder model vs in a variational autoencoder (image credit [3])</figcaption>
</figure>

The training procedure for VAEs is well-established at this point, but it's worth understanding the underlying assumptions that go into formulating the loss function we're actually minimizing. In this post, I'll walk through that derivation step by step and try to build intuition for each piece along the way. I've also put together a short notebook that trains a convolutional VAE on the Fashion-MNIST dataset using PyTorch for anyone who wants to see it in code: [VAE](https://github.com/paymantohidifar/small-ml-projects/blob/main/notebooks/vae.ipynb).

## The Statistical Motivation

From a statistics perspective, a VAE is a Bayesian inference problem. Say we have observed data $x$ (e.g. a molecule fingerprint, an encoded protein sequence, or an image) that we believe was generated from some latent variable $z$ that is unknown to us. We want to learn the distribution over $z$ that our data was generated from. In other words, we'd like the posterior distribution $p(z \mid x)$. By Bayes' theorem:
...
where $p(z)$ is the prior distribution over the latent variable, and $p(x)$ is the marginal likelihood of the data, obtained by marginalizing the joint distribution $p(x,z)$ over $z$. Keep in mind that our ultimate goal is to maximize $p(x)$.

$p(x)$ can be computed by integrating over the entire continuous latent space (or summed, if the latent space is discrete):

$$p(x) = \int_z p(x,z)\,dz = \int_z p(x\mid z)\,p(z)\,dz$$

Computing $p(x)$ is generally intractable (difficult or impossible to compute). So instead of computing the true posterior $p(z \mid x)$ exactly, we approximate it with a tractable distribution $q_{\theta}(z \mid x)$ and try to make $q_{\theta}(z \mid x)$ as close as possible to the true posterior. "As close as possible" needs a similarity measure between distributions, and the standard choice is the Kullback-Leibler (KL) divergence. Here $\theta$ denotes the set of parameters governing the approximate posterior.
...
Now let's go back to our original problem: minimizing the difference between $p(z \mid x)$ and its proxy $q_\theta(z\mid x)$. Let's assume our dataset is comprised of N datapoints: $X = \{x_1, x_2, \dots, x_N\}$. We can write the KL divergence for a single datapoint $x_i$ as:
...
We also assume the generative distribution is specified by a separate set of parameters $\phi$, i.e. $p_\phi(x_i \mid z)$. The final form can be written as:

$$\log p(x_i) \ge \mathbb{E}_{\sim q_{\theta}(z \mid x_i)} \left[\log p_\phi(x_i \mid z)\right] - D_{KL}\big(q_\theta(z \mid x_i)\,\|\,p(z)\big)$$

The right-hand side is the **Evidence Lower BOund (ELBO)**, also called the variational lower bound. As noted earlier, our ultimate goal is to maximize $p(x_i)$. Because $\log$ is a monotonically increasing function, maximizing $\log p(x_i)$ is equivalent to maximizing $p(x_i)$. Maximizing the log-likelihood of our data directly is intractable because it depends on the same troublesome integral as before. The ELBO gives us a lower bound on that quantity that we can compute and optimize instead. That's a maximization problem, but we can turn it into a minimization problem by defining the loss function as the negative of the ELBO:
...
$$p(z) = \mathcal{N}(0, I), \qquad q_\theta(z \mid x_i) = \mathcal{N}\big(\mu,\, \sigma^2 I\big)$$
...
$$D_{KL}\big(q_\theta(z\mid x_i)\,\|\,p(z)\big) = -\frac{1}{2}\sum_{j=1}^{J}\Big(1 + \log \sigma^2_{ij} - \mu^2_{ij} - \sigma^2_{ij}\Big)$$
...
<figure>
  ...
  <figcaption>The reparameterization trick reroutes the stochastic sampling step through a fixed noise source $\epsilon$, keeping the path from $\mu$ and $\sigma$ to $z$ differentiable (image credit [3])</figcaption>
</figure>
...
In the original VAE paper, Kingma & Welling showed that if your mini-batch size is large enough (e.g., 64, or 128 datapoints), setting $L = 1$ (drawing just one random latent sample $z$ per input) provides an unbiased estimate with low enough variance for stable gradient descent. Because we repeat this across a mini-batch of many inputs, the average loss over the batch effectively serves as the expectation operator.

Combining the closed-form KL term with a single-sample Monte Carlo estimate of the reconstruction term, the loss objective will be simplified to:

$$\mathcal{L}(x_i;\theta,\phi) = -\log p_\phi(x_i \mid z) - \frac{1}{2}\sum_{j=1}^{J}\Big(1 + \log \sigma^2_{ij} - \mu^2_{ij} - \sigma^2_{ij}\Big)$$

The loss for the entire dataset is then simply the sum of this per-datapoint loss over all $N$ datapoints:

$$\mathcal{L}(X; \theta,\phi) = \sum_{i=1}^{N} \mathcal{L}(x_i; \theta,\phi)$$

We can construct an estimator of the loss of full dataset, based on mini-batches $X^M$, where $X^M = \{x_i\}_{i=1}^M$:

$$\mathcal{L}(X; \theta,\phi) \approx \mathcal{\widetilde{L}^M}(X^M; \theta,\phi) = \frac{N}{M}\sum_{i=1}^{M} \mathcal{L}(x_i; \theta,\phi)$$

In practice, we run $N/M$ iterations over mini-batches of the dataset prepared using PyTorch `DataLoader`. Ultimately, by training the VAE network, we aim to find encoder and decoder parameters $(\theta^*, \phi^*)$ that jointly minimize $\mathcal{L}(X; \theta,\phi)$ over the whole dataset:

$$(\theta^*, \phi^*) = argmin_{(\theta, \phi)}\mathcal{L}(X; \theta, \phi)$$

There's a second, separate independence assumption worth unpacking inside the reconstruction term itself. Suppose each datapoint has $D$ features (e.g., $D$ pixels in an image, or $D$ amino acids in a protein sequence). In other words, $x_i \in \mathbb{R}^{1\times D}$. Given the latent code $z$, we assume the individual features of $x_i$ are conditionally independent of one another, so the reconstruction term factorizes into a sum of per-feature log-likelihoods:

$$\log p_\phi(x_i \mid z) = \sum_{k=1}^{D} \log p_\phi(x_{i,k} \mid z)$$

In practice, this per-feature term is often implemented as element-wise binary cross-entropy, categorical cross-entropy, or mean squared error, depending on whether the task is classification or regression, respectively.

In the following section, we'll show how the reconstruction loss turns into cross-entropy loss or MSE loss. For brevity, we will drop the datapoint index $i$ and write $x$ for that one datapoint's feature vector of dimension $D$.

### Reconstruction Loss for Binary Classification

We can model a binary feature $x_k$ (e.g. an image pixel that is either 0 or 1) with a Bernoulli distribution. Let's say the decoder network outputs a reconstructed probability $\hat x_k = f_\phi(z)_k$ for an input feature $x_k = 1$ using a sigmoid activation function. The Probability Mass Function (PMF) for a single Bernoulli random variable is:
...
Since the reconstruction loss is defined as $-\log p_\phi(x_k \mid z)$, negating both sides gives $-x_k \log \hat x_k - (1 - x_k) \log (1 - \hat x_k)$, which is exactly the standard binary cross-entropy (BCE) loss. Summed over all $D$ pixels (features), this is the reconstruction loss for a datapoint implemented in practice.
...
For a continuous feature, we assume it follows a Gaussian distribution with a fixed variance. Below, we'll show how this assumption turns the reconstruction loss into an MSE loss problem. Again, suppose our continuous datapoint $x$ has $D$ features. We assume that given the latent code $z$, each feature $x_k$ is normally distributed around a mean $\mu_k$ predicted by the decoder, with some fixed variance $\sigma^2$:
...
Here, the decoder network outputs the mean $\hat x_k = \mu_k = f_\phi(z)_k$ (usually using a linear or identity activation function so the values can be any real number.) Assuming independence across all $D$ features, the joint probability is the product of the individual per-feature densities. Taking the natural log converts the product into a sum:
...
When training a neural network, any term that doesn't depend on the network's weights $\phi$ can be dropped, since its derivative with respect to the weights is zero. Therefore, the term $\frac{1}{2}\log(2\pi\sigma^2)$ will be dropped and leaves us with the core optimization term:

$$\mathcal{L}_{\text{recon}} \propto \sum_{k=1}^{D} (x_k - \mu_k)^2 = \sum_{k=1}^{D} (x_k - \hat x_k)^2$$
...
At the end, I hope this post helped you understand the math behind implementing the loss objective in practice and build intuition for it.
